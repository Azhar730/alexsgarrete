...
function SuccessContent() {
  ...
  return (
    <div className="bg-white p-6 sm:p-8 md:p-10 rounded-2xl sm:rounded-3xl shadow-none max-w-lg w-full text-center border border-slate-100 relative overflow-hidden">
      <div className="absolute -top-16 left-1/2 -translate-x-1/2 w-40 h-40 bg-emerald-50/70 rounded-full blur-3xl -z-10" />
      ...
      <div className="w-20 h-20 sm:w-24 sm:h-24 bg-linear-to-tr from-emerald-400 to-emerald-500 text-white rounded-full flex items-center justify-center mx-auto mb-6 sm:mb-8 shadow-none relative">
        <CheckCircle2 className="w-10 h-10 sm:w-12 sm:h-12" strokeWidth={2.25} />
      </div>
      ...
      <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-slate-900 mb-3 sm:mb-4 tracking-tight leading-tight">Payment Successful!</h1>
      ...
      {sessionId && (
        <div className="bg-slate-50 rounded-xl p-3 sm:p-4 mb-6 sm:mb-8 border border-slate-100 max-w-sm mx-auto">
          ...
          <p className="text-[11px] sm:text-xs font-mono text-slate-600 truncate px-1 sm:px-2">{sessionId}</p>
        </div>
      )}
