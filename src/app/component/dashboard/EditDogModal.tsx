...
// ─── Default Values Type ──────────────────────────────────────
export interface EditDogDefaults {
  name: string;
  gender: string;
...
export function EditDogModal({
  ...
  const [imagePreview, setImagePreview] = useState<string | null>(
    defaultValues.currentImagePreview || null
  );
  ...
  // Re-sync form when defaultValues change (e.g., modal re-opens)
  useEffect(() => {
    if (open) {
      form.reset({
        name: defaultValues.name,
      ...
      setImagePreview(defaultValues.currentImagePreview || null);
      setImageFile(null);
      setImageError(null);
    }
      ...
      <DialogContent className="sm:max-w-[720px] max-h-[92vh] p-0 gap-0 rounded-3xl border-none shadow-2xl flex flex-col overflow-hidden">
        ...
        <Form {...form}>
          ...
          >
            {/* ── Scrollable Content ── */}
            <div
              className="flex-1 overflow-y-auto px-8 py-7 space-y-8"
              data-lenis-prevent
            >
              {/* ── Photo Upload ── */}
