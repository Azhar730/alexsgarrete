...
import { useGetActiveQuestionnaireQuery, useGiveAnswerMutation, useFamilyHealthHistoryMutation } from "@/redux/api/onboardingApi";
    ...
    questionnaire?.questions?.reduce<Record<string, QuestionAnswer>>(
      (acc, q) => {
        ...
        acc[q.id] = {
          ...
          cancerRelation: existing?.cancerRelation ?? "",
          cancerDiagnosis: existing?.cancerDiagnosis ?? "",
          cancerAgeOnset: existing?.cancerAgeOnset ?? "",
          cancerAgeAtDeath: existing?.cancerAgeAtDeath ?? "",
          tobaccoCurrentUser: existing?.tobaccoCurrentUser,
          tobaccoLastUsed: existing?.tobaccoLastUsed ?? "",
        };
        return acc;
      },
      {}
...
function mapToHealthDetails(
  ...
  const cancerQ = questionnaire?.questions?.find((q) => q.category === "CANCER");
...
export function StepHealthDetails({ applicationId }: { applicationId?: string }) {
  ...
  const defaultValues = useMemo(
    () => buildDefaultAnswers(questionnaire, data.healthDetails),
    [data.healthDetails, questionnaire]
  );
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      const entries = Object.entries(watchedAnswers);
      for (const [questionId, answer] of entries) {
        ...
        if (answer?.answer !== undefined) {
          try {
            await giveAnswer({
              ...
              inputValue:
                // prefer explicit nested explanation or cancerDiagnosis or tobaccoLastUsed
                (answer.nested?.explanation as string) || answer.cancerDiagnosis || answer.tobaccoLastUsed || undefined,
            }).unwrap();
          } catch (e) {
        ...
        if (q.category === "CANCER" && answer?.answer === "Yes" && answer.cancerDiagnosis) {
          try {
            await familyHealthHistory({
              questionId,
              relation: answer.cancerRelation || "",
              diagnosis: answer.cancerDiagnosis || "",
              approxAgeOfOnset: answer.cancerAgeOnset ? parseInt(String(answer.cancerAgeOnset)) : undefined,
              ageAtDeath: answer.cancerAgeAtDeath ? parseInt(String(answer.cancerAgeAtDeath)) : undefined,
            }).unwrap();
          } catch (e) {
  ...
  }, [watchedAnswers, applicationId, questionnaire, giveAnswer, familyHealthHistory]);
